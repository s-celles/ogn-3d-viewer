...
- **Lift potential** *(opt-in, off by default, experimental)* — a physics estimate of **where the air rises**: **thermal** (sun × terrain slope × heat flux → w\*, with OSM land-cover albedo/Bowen and cast shadows), **slope lift** (wind × terrain) and **convergence** (wind × terrain curvature: air piling up at valley heads and confluences), **balanced with a simplex mixer** (an axis for 2 components, a triangle for 3, a polygon beyond — thermal, slope, convergence; wave to come): the nearer a vertex, the more that component dominates. A warm (climbs) / blue (sinks) field draped on the terrain, colour-coherent across components. **Very approximate model** (see *Notes & limitations*).
