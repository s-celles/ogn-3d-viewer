...
- **Lift potential** *(opt-in, off by default, experimental)* — a physics estimate of **where the air rises**: **thermal** (sun × terrain slope × heat flux → w\*, with OSM land-cover albedo/Bowen) and **slope lift** (wind × terrain), **balanced with a simplex mixer** (an axis for 2 components, a triangle for 3 — thermal, slope; wave to come): the nearer a vertex, the more that component dominates. A warm (climbs) / blue (sinks) field draped on the terrain: a thermal sheet and the slope bands, colour-coherent. **Very approximate model** (see *Notes & limitations*).
...
- **Air mass — a very rough, illustrative model** (neither measured nor predictive; not for flight planning):
  ...
  - *Lift potential* — a **physics** estimate, not a measurement, overlaying the checked components. *Thermal* (w\*): approximate albedo/Bowen (OSM land-cover, unavailable if Overpass is down → uniform), no cumulus shading or advection; an **ensemble mean**, sharpest at **low sun**. Rendered as a **fractional anomaly vs a flat reference patch** under the same sun (mass continuity, and **view-independent** — a slope keeps its colour as the camera moves): better-heated-than-flat is warm, worse (shaded faces, bright/valley ground) is blue **compensating subsidence** — not a measured downdraught. *Slope* (`w = wind · ∇terrain`, refined by an upwind-sheltering heuristic): a **first-order kinematic approximation** ignoring flow separation, rotor, lee waves and stability, with one wind for the whole scene and detail limited by the DEM; blue (sink) comes from **lee slopes**.
