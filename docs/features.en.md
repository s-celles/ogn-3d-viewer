...
- **Lift potential** *(opt-in, off by default, experimental)* — a physics estimate of **where the air rises**: **thermal** (sun × terrain slope × heat flux → w\*, with OSM land-cover albedo/Bowen) and **slope lift** (wind × terrain), **blended** by a slider. A warm field (it climbs) draped on the terrain, colour-coherent with the slope bands. **Very approximate model** (see *Notes & limitations*).
