...
- **Wave scan** — the **🌊** tab ranks the known spots by **mountain-wave potential** for the chosen date: it batch-fetches the upper wind and stability (Open-Meteo) and scores each site (cross-ridge wind × stability × a plausible wavelength × the site's **relief** and whether the **wind crosses a ridge** — so flat plains are dropped, and a site with several ridges is handled). *Weather first, then pick the field* — click a site to load it on a promising wave day.
