...
- **Lift potential** *(opt-in, off by default, experimental)* — a physics estimate of **where the air rises**: **thermal** (sun × terrain slope × heat flux → w\*, with OSM land-cover albedo/Bowen) and **slope lift** (wind × terrain), **toggled by checkboxes** (thermal, slope; wave to come) and summed. A warm (climbs) / blue (sinks) field draped on the terrain, colour-coherent with the slope bands. **Very approximate model** (see *Notes & limitations*).
...
- **Air mass — a very rough, illustrative model** (neither measured nor predictive; not for flight planning):
  ...
  - *Lift potential* — a **physics** estimate (thermal w\* + slope), not a measurement: approximate albedo/Bowen (OSM land-cover, unavailable if Overpass is down → uniform), no cumulus shading or advection; an **ensemble mean**. Blue (sink) comes from **lee slopes**; sharpest at **low sun**.
