...
- **Local file import** — drop or pick your own **IGC / GPX / KML** tracks (or a SeeYou **`.cup`** waypoint file — see *Points of interest* — or an XCSoar/LK8000 **`.plr`** glider polar — see *Netto vario*) to replay them the same way, without going through OGN.
...
- **Workable cols** *(opt-in)* — mark the mountain passes (DEM saddles) the wind blows **through** — where it funnels (venturi) and slope lift builds on the upwind approach. Each shows a small flow-over-the-col glyph: it **climbs the windward slope (green = lift, the workable side)** then **sinks on the lee (blue = subsidence/rotor)**, so the side to work is unambiguous. Only appears where the wind actually crosses the pass (so it reacts as you change the sim wind).
...
- **Lift potential** *(opt-in, off by default, experimental)* — a physics estimate of **where the air rises**: **thermal** (sun × terrain slope × heat flux → w\*, with OSM land-cover albedo/Bowen, cast shadows, a **convex-break trigger** bias — favouring ridges and convex slope breaks over the flat surface — and **diurnal soil heat storage** whose per-surface inertia shifts the peak into the afternoon and keeps rock/urban thermals going late, adjustable with a slider; on a windy convective day the thermals organise into **cloud streets** aligned with the wind), **slope lift** (wind × terrain, plus a sunny-day **anabatic** upslope component so ridges work even in calm) and **convergence** (wind × terrain curvature: air piling up at valley heads and confluences, plus a **lake/sea-breeze** front a few km inland of water on hot days), each component is **toggled by a checkbox**, then the enabled ones are **balanced with a simplex mixer** whose shape follows their count (an axis for 2, a triangle for 3, a polygon beyond — thermal, slope, convergence, wave): the nearer a vertex, the more that component dominates. A warm (climbs) / blue (sinks) field draped on the terrain, colour-coherent across components. On a **cumulus day** the strongest thermal cores are topped with a **cumulus** at the cloudbase (none on a blue day). A **colour legend** in the panel maps the ramp to sink/lift, with a Vz anchor for the thermal. A **day-structure** panel (emagram: sounding, parcel, cloudbase, ceiling — with the convective depth and cu/blue) helps read the day. **Very approximate model** (see *Notes & limitations*).
...
- **Weather sandbox** *(opt-in, off by default)* — replace the fetched weather with a **synthetic atmosphere** (wind speed / direction / shear, stability N, surface temp, humidity) and a chosen **sun date/time**, to explore *what-if* situations — e.g. force a lee-wave day and watch the Onde sheets react: stacked and steady when the sounding says the wave is **trapped**, tilted and fading with height when it's **propagating** instead, or just a single low sheet — with a small on-screen note — when the regime can't be told apart. Ragged **rotor** roll-clouds flag likely turbulence low under the crests (an indicative hazard cue, not a computed rotor — see its tooltip); the day-structure emagram and the wave scan react too. It feeds **every** physics model; One-click **presets** (thermal / cumulus / ridge / wave / low-sun days, each needing its own atmosphere) and a reset are provided; a **⚠ simulated** badge marks the mode. Illustrative, not a forecast.
...
- **Air mass — a very rough, illustrative model** (neither measured nor predictive; not for flight planning):
  - *Thermals* — shown only where a glider actually **circled** (no traffic → nothing); their position and strength are the **glider's climb**, not the true air motion (no *netto*); weak or brief lift may be missed. **Wave climbs** (ribbons) are detected by the opposite — a sustained, **straight** climb above the terrain — and may mistake a long ridge run for wave. Aerotows are filtered out where they'd otherwise look like wave (a climb-rate ceiling, a companion aircraft within rope distance, or a low start ending in a release) — not perfect: a genuine climb right off a low release can still be missed, favouring silence over painting a tow as wave.
