...
- **Weather sandbox** *(opt-in, off by default)* — replace the fetched weather with a **synthetic atmosphere** (wind speed / direction / shear, stability N, surface temp, humidity) and a chosen **sun date/time**, to explore *what-if* situations — e.g. force a lee-wave day and watch the Onde bands, the day-structure emagram and the wave scan react. It feeds **every** physics model; One-click **presets** (thermal / cumulus / ridge / wave / low-sun days, each needing its own atmosphere) and a reset are provided; a **⚠ simulated** badge marks the mode. Illustrative, not a forecast.
