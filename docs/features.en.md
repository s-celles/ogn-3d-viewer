...
- **3D buildings (OSM)** *(opt-in)* — extrude OpenStreetMap building footprints around the view: walls (lit for depth) + flat roofs, height from the `height` / `building:levels` tags (else a default). "Autogen" city context — clean extruded blocks, not photogrammetry. Only when zoomed in (city scale), radius-limited and capped for performance; best in well-mapped regions (e.g. Switzerland). Rough & illustrative.
...
- **Weather sandbox** *(opt-in, off by default)* — replace the fetched weather with a **synthetic atmosphere** (wind speed / direction / shear, stability N, surface temp, humidity) and a chosen **sun date/time**, to explore *what-if* situations — e.g. force a lee-wave day and watch the Onde bands (with ragged **rotor** roll-clouds flagging the turbulence low under the crests), the day-structure emagram and the wave scan react. It feeds **every** physics model; One-click **presets** (thermal / cumulus / ridge / wave / low-sun days, each needing its own atmosphere) and a reset are provided; a **⚠ simulated** badge marks the mode. Illustrative, not a forecast.
