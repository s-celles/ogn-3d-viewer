...
- **Workable cols** *(opt-in)* — mark the mountain passes (DEM saddles) the wind blows **through** — where it funnels (venturi) and slope lift builds on the upwind approach. Each shows a ridge tick and a **through-wind arrow**, green when the through-wind is strong, and only appears when the wind actually crosses the pass (so it reacts as you change the sim wind).
