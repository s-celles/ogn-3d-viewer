...
- **Wind** *(opt-in, off by default, experimental)* — visualises the wind field, **local and terrain-refined**, resolved **by altitude** (Open-Meteo profile). A dropdown picks the representation: **Draped (2D)** in 3 variants (vectors, speed colours, or both), **Wind barbs (2D)** (met convention: half barb 5 kt, full 10 kt, pennant 50 kt), **Altitude layers (3D)**, **Rings per altitude (3D)**, **Hodograph (3D)** (profile spiral → shear). A **rose** (corner) gives speed and source. **Very approximate** (see *Notes & limitations*).
