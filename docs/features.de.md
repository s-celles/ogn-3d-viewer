...
- **Aufwindpotenzial** *(optional, standardmäßig aus, experimentell)* — physikalische Schätzung, **wo die Luft steigt**: **Thermik** (Sonne × Hang × Wärmefluss → w\*, mit OSM-Landbedeckung Albedo/Bowen und Schlagschatten), **Hangaufwind** (Wind × Gelände) und **Konvergenz** (Wind × Geländekrümmung: Luft staut sich an Talschlüssen und Zusammenflüssen), **mit einem Simplex-Mischpult dosiert** (eine Achse für 2 Komponenten, ein Dreieck für 3, ein Polygon darüber hinaus — Thermik, Hang, Konvergenz; Welle folgt): je näher an einer Ecke, desto stärker diese Komponente. Warmes (steigt) / blaues (sinkt) Feld über das Gelände gelegt, farblich stimmig zwischen den Komponenten. **Sehr grobes Modell** (siehe *Hinweise & Grenzen*).
