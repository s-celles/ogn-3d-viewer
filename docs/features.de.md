...
- **Aufwindpotenzial** *(optional, standardmäßig aus, experimentell)* — physikalische Schätzung, **wo die Luft steigt**: **Thermik** (Sonne × Hang × Wärmefluss → w\*, mit OSM-Landbedeckung Albedo/Bowen) und **Hangaufwind** (Wind × Gelände), **mit einem Simplex-Mischpult dosiert** (eine Achse für 2 Komponenten, ein Dreieck für 3 — Thermik, Hang; Welle folgt): je näher an einer Ecke, desto stärker diese Komponente. Warmes (steigt) / blaues (sinkt) Feld über das Gelände gelegt: eine Thermikfläche und die Hangbänder, farblich stimmig. **Sehr grobes Modell** (siehe *Hinweise & Grenzen*).
...
- **Luftmasse — sehr grobes, rein illustratives Modell** (weder gemessen noch vorhersagend; nicht zur Flugvorbereitung):
  ...
  - *Aufwindpotenzial* — eine **physikalische** Schätzung, keine Messung, Überlagerung der aktivierten Komponenten. *Thermik* (w\*): näherungsweise Albedo/Bowen (OSM-Landbedeckung, ohne Overpass → uniform), keine Cumulus-Beschattung/Advektion; ein **Ensemble-Mittel**, am deutlichsten bei **tiefer Sonne**. Als **fraktionale Anomalie gegenüber einer flachen Referenzfläche** unter derselben Sonne dargestellt (Massenerhaltung, und **blickpunktunabhängig** — ein Hang behält seine Farbe, wenn die Kamera schwenkt): stärker erwärmt als flach warm, schwächer (beschattete Hänge, heller Boden/Talböden) blau als **kompensierendes Absinken** — kein gemessener Abwind. *Hang* (`w = Wind · ∇Gelände`, verfeinert durch eine Luv-Abschattungsheuristik): eine **kinematische Näherung erster Ordnung**, die Strömungsablösung, Rotor, Leewellen und Stabilität ignoriert, mit einem Wind für die ganze Szene und durch die DEM begrenztem Detail; Blau (Abwind) von **Leehängen**.
