...
- **Nutzbare Pässe** *(optional)* — markiert die vom Wind **durchströmten** Pässe (DEM-Sättel), wo er sich staut (Venturi) und luvseitig Hangaufwind entsteht. Jeder zeigt einen Kammstrich und einen **Durchwind-Pfeil**, grün bei starkem Durchwind, und erscheint nur, wenn der Wind den Pass wirklich quert (reagiert also auf den simulierten Wind).
