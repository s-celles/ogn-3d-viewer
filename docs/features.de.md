...
- **Aufwindpotenzial** *(optional, standardmäßig aus, experimentell)* — physikalische Schätzung, **wo die Luft steigt**: **Thermik** (Sonne × Hang × Wärmefluss → w\*, mit OSM-Landbedeckung Albedo/Bowen) und **Hangaufwind** (Wind × Gelände), per **Kontrollkästchen wählbar** (Thermik, Hang; Welle folgt) und summiert. Warmes (steigt) / blaues (sinkt) Feld über das Gelände gelegt, farblich stimmig mit den Hangbändern. **Sehr grobes Modell** (siehe *Hinweise & Grenzen*).
...
- **Luftmasse — sehr grobes, rein illustratives Modell** (weder gemessen noch vorhersagend; nicht zur Flugvorbereitung):
  ...
  - *Aufwindpotenzial* — eine **physikalische** Schätzung (Thermik w\* + Hang), keine Messung: näherungsweise Albedo/Bowen (OSM-Landbedeckung, ohne Overpass → uniform), keine Cumulus-Beschattung/Advektion; ein **Ensemble-Mittel**. Blau (Abwind) von **Leehängen**; am deutlichsten bei **tiefer Sonne**.
