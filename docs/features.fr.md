...
- **Simulateur météo** *(option, désactivé par défaut)* — remplace la météo récupérée par une **atmosphère synthétique** (vent vitesse / direction / cisaillement, stabilité N, température surface) et une **date/heure de soleil** au choix, pour explorer des situations « et si… » — p. ex. forcer un jour d'onde et voir réagir les bandes Onde, l'émagramme du jour et le scan d'onde. Irrigue **tous** les modèles physiques ; Des **préréglages Jour thermique / Jour d'onde** (les deux exigent des atmosphères opposées — stable + venté pour l'onde, instable + chaud pour le thermique) et un reset sont fournis ; un bandeau **⚠ simulé** signale le mode. Illustratif, pas une prévision.
