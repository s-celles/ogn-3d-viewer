...
- **Potentiel de portance** *(option, désactivée par défaut, expérimental)* — estime par la physique **où l'air monte** : **thermique** (soleil × pente du relief × flux de chaleur → w\*, avec albédo/Bowen par type de sol OSM) et **portance de pente** (vent × relief), **activables par cases à cocher** (thermique, pente ; onde à venir) et sommés. Champ chaud (ça monte) / bleu (ça descend) drapé sur le relief, cohérent en couleur avec les bandes de pente. **Modèle très approximatif** (voir *Notes & limites*).
...
- **Masse d'air — modèle très approximatif, purement illustratif** (ni mesuré ni prédictif ; à ne pas utiliser pour préparer un vol) :
  ...
  - *Potentiel de portance* — estimation **physique** (thermique w\* + pente), pas une mesure : albédo/Bowen approchés (land‑cover OSM, indispo si Overpass ne répond pas → uniforme), pas d'ombre des cumulus ni d'advection ; **champ moyen**. Le bleu (chute) vient des **faces sous le vent** ; contraste maximal **soleil bas**.
