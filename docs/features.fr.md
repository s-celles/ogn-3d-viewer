...
- **Scan d'onde** — l'onglet **🌊** classe les spots connus par **potentiel d'onde de ressaut** pour la date choisie : il récupère en une requête le vent d'altitude et la stabilité (Open-Meteo) et note chaque site (vent traversier × stabilité × longueur d'onde plausible × le **relief** du site et si le **vent traverse une crête** — les plaines sont écartées, et un site à plusieurs crêtes est géré). *La météo d'abord, le terrain ensuite* — cliquez un site pour le charger un jour d'onde prometteur.
