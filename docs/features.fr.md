...
- **Potentiel de portance** *(option, désactivée par défaut, expérimental)* — estime par la physique **où l'air monte** : **thermique** (soleil × pente du relief × flux de chaleur → w\*, avec albédo/Bowen par type de sol OSM) et **portance de pente** (vent × relief), **dosés par un mélangeur simplexe** (un axe pour 2 composantes, un triangle pour 3 — thermique, pente ; onde à venir) : plus on approche un sommet, plus sa composante domine. Champ chaud (ça monte) / bleu (ça descend) drapé sur le relief : nappe thermique et bandes de pente, cohérentes en couleur. **Modèle très approximatif** (voir *Notes & limites*).
...
- **Masse d'air — modèle très approximatif, purement illustratif** (ni mesuré ni prédictif ; à ne pas utiliser pour préparer un vol) :
  ...
  - *Potentiel de portance* — estimation **physique**, pas une mesure, superposition des composantes cochées. *Thermique* (w\*) : albédo/Bowen approchés (land‑cover OSM, indispo si Overpass ne répond pas → uniforme), pas d'ombre des cumulus ni d'advection ; **champ moyen**, contraste maximal **soleil bas**. Affiché en **anomalie fractionnaire vs un sol plat de référence** sous le même soleil (conservation de la masse, et **indépendant du point de vue** — une pente garde sa couleur quand la caméra bouge) : mieux chauffé que le plat en chaud, moins bien (faces à l'ombre, sol clair/fonds de vallée) en bleu, **subsidence compensatoire** — pas une dégueulante mesurée. *Pente* (`w = vent · ∇relief`, affiné par une heuristique d'abri au vent) : **approximation cinématique au 1er ordre** ignorant décollements, rotors, ondes de ressaut et stabilité, avec un seul vent pour toute la scène et une finesse limitée par le MNT ; le bleu (chute) vient des **faces sous le vent**.
