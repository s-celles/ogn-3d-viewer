...
- **Simulateur météo** *(option, désactivé par défaut)* — remplace la météo récupérée par une **atmosphère synthétique** (vent vitesse / direction / cisaillement, stabilité N, température surface) et une **date/heure de soleil** au choix, pour explorer des situations « et si… » — p. ex. forcer un jour d'onde et voir réagir les bandes Onde, l'émagramme du jour et le scan d'onde. Irrigue **tous** les modèles physiques ; un bandeau **⚠ simulé** signale le mode. Illustratif, pas une prévision.
