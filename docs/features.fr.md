...
- **Scan d'onde** — l'onglet **🌊** classe les spots connus par **potentiel d'onde de ressaut** pour la date choisie : il récupère en une requête le vent d'altitude et la stabilité (Open-Meteo) et note chaque site (vent traversier × stabilité × longueur d'onde plausible × le **relief** du site et si le **vent traverse une crête** — les plaines sont écartées, et un site à plusieurs crêtes est géré). Il liste les terrains d'onde et note chacun de la **chance du jour** (exploitable / possible / faible), donc jamais vide. Chaque spot des listes porte aussi un **tag de terrain** — plaine 🌾 / collines 🏞 / montagne ⛰ / haute montagne 🏔 — classé à la **Kapos/Meybeck** d'après l'amplitude locale du relief (≤ 12 km) et l'altitude des sommets de la région (haute montagne ≥ 2500 m). La *chance* d'onde est à part, dynamique. *La météo d'abord, le terrain ensuite* — cliquez un site pour le charger un jour d'onde prometteur.
...
- **Import de fichiers locaux** — glissez ou choisissez vos propres traces **IGC / GPX / KML** (ou un fichier de points **`.cup`** SeeYou — voir *Points d'intérêt* — ou une polaire de planeur **`.plr`** XCSoar/LK8000 — voir *Vario Netto*) pour les rejouer de la même façon, sans passer par OGN.
...
- **Téléportation (🛰)** — largue une caméra observateur libre n'importe où : saisis un **`lat, lon`**, un **code d'aérodrome** ou un **nom de sommet/waypoint**, règle une **altitude** (ou une **hauteur sol**) et un **cap initial**, et saute sur place pour regarder autour (glisser pour pivoter/incliner). Le HUD affiche la position de l'observateur, sa hauteur sol et le vent à ce point. Fonctionne sans aucun vol chargé (idéal pour le simulateur météo) ; un bouton de vue te ramène.
...
- **Cols exploitables** *(optionnel)* — marque les cols (points de selle du MNT) que le vent **traverse** — là où il s'engouffre (venturi) et où la portance de pente se construit à l'amont. Chacun montre un petit glyphe de flux par-dessus le col : il **monte le versant au vent (vert = portance, le côté exploitable)** puis **redescend au vent arrière (bleu = subsidence/rotor)**, pour lever l'ambiguïté sur le bon côté. N'apparaît que là où le vent franchit réellement le col (donc réagit quand tu changes le vent simulé).
...
- **Simulateur météo** *(option, désactivé par défaut)* — remplace la météo récupérée par une **atmosphère synthétique** (vent vitesse / direction / cisaillement, stabilité N, température surface, humidité) et une **date/heure de soleil** au choix, pour explorer des situations « et si… » — p. ex. forcer un jour d'onde et voir réagir les bandes Onde (avec des nuages de **rotor** déchiquetés signalant la turbulence bas sous les crêtes), l'émagramme du jour et le scan d'onde. Irrigue **tous** les modèles physiques ; Des **préréglages** (jour thermique / cumulus / vol de pente / onde / soleil bas, chacun avec son atmosphère) et un reset sont fournis ; un bandeau **⚠ simulé** signale le mode. Illustratif, pas une prévision.
