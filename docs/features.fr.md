...
- **Potentiel de portance** *(option, désactivée par défaut, expérimental)* — estime par la physique **où l'air monte** : **thermique** (soleil × pente du relief × flux de chaleur → w\*, avec albédo/Bowen par type de sol OSM et ombres portées), **portance de pente** (vent × relief) et **convergence** (vent × courbure du relief : l'air s'accumule aux têtes de vallée et confluences), **dosés par un mélangeur simplexe** (un axe pour 2 composantes, un triangle pour 3, un polygone au-delà — thermique, pente, convergence ; onde à venir) : plus on approche un sommet, plus sa composante domine. Champ chaud (ça monte) / bleu (ça descend) drapé sur le relief, cohérent en couleur entre composantes. **Modèle très approximatif** (voir *Notes & limites*).
