...
const CFG = 'ogn-tiles-cfg';   // holds the persisted tile-cap (not counted/trimmed)
const CAP_URL = 'https://ogn.local/tilemax';
// Persistent tile cache cap (~30 KB/tile on disk). Defaults by device RAM; the
// page overrides it at runtime via postMessage (user 'cache size' setting) and
...
self.addEventListener('message', (e) => {
  if (e.data && e.data.type === 'tileMax' && e.data.value > 0) {
    tileMax = Math.max(100, e.data.value | 0);
    e.waitUntil((async () => { const c = await caches.open(CFG); await c.put(CAP_URL, new Response(String(tileMax))); await trim(); })());
  }
});
self.addEventListener('install', () => self.skipWaiting());
self.addEventListener('activate', (e) => e.waitUntil((async () => {
  const keys = await caches.keys();
  await Promise.all(keys.filter((k) => k !== CACHE && k !== TILES && k !== CFG).map((k) => caches.delete(k)));
  try { const r = await (await caches.open(CFG)).match(CAP_URL); if (r) tileMax = Math.max(100, parseInt(await r.text(), 10) || tileMax); } catch (e) {}
