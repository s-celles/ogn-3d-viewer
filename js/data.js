...
import { API_BASE, PALETTE } from './config.js';
import { parseTz, parseIGC, pool } from './igc.js';
import { t } from './i18n.js';
import { statusEl, loadBtn, subjEl, viewsEl, playBtn, icaoEl } from './dom.js';
import { render } from './render.js';
import { buildLegend, syncUI, applyFollowClass, setCollapsed } from './ui.js';

// Stable colour per registration, recycled across the palette.
export function colorFor(reg){ if(!(reg in S.COLOR)) S.COLOR[reg]=PALETTE[S.colorN++ % PALETTE.length]; return S.COLOR[reg]; }

export function setStatus(msg, cls){ statusEl.className='status'+(cls?(' '+cls):''); statusEl.textContent=msg; }

export async function fetchData(icao, date, onlyActive){
...
function statusMsgInner(date, n){
  let msg = n+' '+t('flights');
...
export async function loadFlights(icao, date){
...
export function rebuild(af, tzoff, preserve){
  ...
  S.AF={name:S.CURAF.name, code:S.CURAF.code, lon:S.CURAF.latlng[1], lat:S.CURAF.latlng[0], elev:S.CURAF.elevation||0, tz_off:S.CURTZ};
  S.G0=g0; S.G1=g1; S.SPAN=Math.max(1,S.G1-S.G0);
  S.TRACKS = tracks.map(tr=>({...tr,
    rel: tr.path.map(p=>[p[0],p[1],p[2],p[3]-S.G0]), rstart:tr.tstart-S.G0, rend:tr.tend-S.G0}));
  if(!preserve){
  ...
  subjEl.innerHTML='';
  S.TRACKS.forEach(tr=>{ const o=document.createElement('option'); o.value=tr.reg;
    o.textContent=`${tr.reg} — ${tr.label}`; subjEl.appendChild(o); });
  subjEl.value=S.subject;
  S.ready=true; document.body.classList.add('loaded');
  [...viewsEl.children].forEach(c=>c.classList.toggle('on', c.dataset.m===S.mode));
...
export async function refreshLive(){
  ...
  try{
    ...
    if(res && res.tracks.length){
      const byReg = new Map(S.RAW.map(tr=>[tr.reg,tr]));
