...
import { API_BASE, REPO_URL, MINZ, MAXZ, PMIN, PMAX, clampv } from './config.js';
import {
  statusEl, subjEl, viewsEl, cammodeEl, traceEl, winEl, winval, playBtn, segEl,
  exoEl, exval, pitchEl, pitchval, scrub, clkEl, lglist, rose, icaoEl, acEl,
  dateEl, loadBtn, langEl, discEl, infoBtn, collapseBtn, liveBtn
} from './dom.js';
import { subjectTrack, airborne, headingAt, clampCur, fmt } from './flight-math.js';
import { makeTerrain } from './terrain.js';
import { render, updateHUD } from './render.js';
import { loadFlights, refreshLive, statusMsg, setStatus } from './data.js';

// ---- clock / scrubber ----
export function syncUI(){ clkEl.textContent= S.ready?fmt(S.cur):'--:--'; scrub.value=Math.round(S.cur/S.SPAN*1000); }
scrub.addEventListener('input', e=>{ if(!S.ready)return; S.cur=e.target.value/1000*S.SPAN; render(); syncUI(); });

// ---- view toggle ----
['over','fpv'].forEach(m=>{ const b=document.createElement('button'); b.dataset.m=m; if(m===S.mode)b.classList.add('on');
  b.onclick=()=>setMode(m); viewsEl.appendChild(b); });
export function setMode(m){ if(m==='fpv'&&!S.ready) return; S.mode=m;
...
export function applyFollowClass(){ document.body.classList.toggle('follow', S.fpvFollow);
  document.body.classList.toggle('free', !S.fpvFollow);
  [...cammodeEl.children].forEach(c=>c.classList.toggle('on',(c.dataset.f==='1')===S.fpvFollow)); }
function setFollow(v){ if(!v){ const tr=subjectTrack(); S.freeCam={bearing:headingAt(tr,clampCur(tr)),pitch:S.fpvPitch}; }
...
subjEl.addEventListener('change', e=>{ S.subject=e.target.value; const tr=subjectTrack();
...
traceEl.addEventListener('change', e=>{ S.trace=e.target.value;
  document.body.classList.toggle('win', S.trace==='window'); render(); });
winEl.addEventListener('input', e=>{ S.windowMin=parseFloat(e.target.value); winval.textContent=S.windowMin; render(); });
...
[1,8,30,120].forEach(s=>{ const b=document.createElement('button'); b.textContent=s+'×'; if(s===S.speed)b.classList.add('on');
  b.onclick=()=>{ S.speed=s; [...segEl.children].forEach(c=>c.classList.remove('on')); b.classList.add('on'); }; segEl.appendChild(b); });

// ---- exaggeration & pitch ----
exoEl.addEventListener('input', e=>{ S.exo=parseFloat(e.target.value); exval.textContent=S.exo.toFixed(1)+'×'; S.terrainInst=makeTerrain(); render(); });
pitchEl.addEventListener('input', e=>{ S.fpvPitch=parseFloat(e.target.value); pitchval.textContent=(S.fpvPitch>=0?'+':'')+S.fpvPitch+'°'; render(); });

document.getElementById('reset').onclick=()=>{ if(S.mode==='over'){ S.mapTarget={...S.INIT}; }
  else { S.fpvPitch=6; pitchEl.value=6; pitchval.textContent='+6°'; setFollow(true); } };

// ---- legend ----
export function buildLegend(){ lglist.innerHTML='';
  S.TRACKS.forEach(tr=>{ const d=document.createElement('div'); d.className='lg'; const dur=Math.round((tr.rend-tr.rstart)/60);
    ...
    d.onclick=()=>{ if(S.mode==='fpv'){ S.subject=tr.reg; subjEl.value=tr.reg; const tt=subjectTrack(); if(!airborne(tt,S.cur)) S.cur=tt.rstart; }
      else { S.solo=(S.solo===tr.reg)?null:tr.reg; [...lglist.children].forEach((c,i)=>c.style.opacity=(S.solo&&S.TRACKS[i].reg!==S.solo)?0.45:1); }
      render(); };
    lglist.appendChild(d); }); }

// ---- navigation ----
export function easeCamera(){ const e=0.25, d=((S.mapTarget.bearing-S.mapVS.bearing+540)%360)-180;
  S.mapVS={...S.mapVS, longitude:S.mapVS.longitude+(S.mapTarget.longitude-S.mapVS.longitude)*e,
...
export function updateCompass(){ if(rose) rose.setAttribute('transform','rotate('+(-(S.mapVS.bearing||0))+' 20 20)'); }
const NAV={ rotL:()=>S.mapTarget.bearing-=30, rotR:()=>S.mapTarget.bearing+=30,
...
document.getElementById('compass').onclick=()=>{ S.mapTarget.bearing=0; };
...
function stopLive(){ S.live=false; clearTimeout(S.liveTimer); S.liveTimer=null;
...
async function setLive(){
  ...
  if(S.live){ if(S.ready) statusMsg(dateEl.value, S.RAW.length); else setStatus(t('noFlights'));
    clearTimeout(S.liveTimer); S.liveTimer=setTimeout(refreshLive, 20000); } // keep polling (waits for first flight too)
}
liveBtn.onclick=setLive;

// ---- collapse panel (keeps the map visible, esp. on phones) ----
export function setCollapsed(c){ document.body.classList.toggle('collapsed',c);
...
export function applyI18n(){
  ...
  [...langEl.children].forEach(b=>b.classList.toggle('on', b.dataset.l===S.lang));
...
addEventListener('keydown', e=>{
  ...
  else if('123'.includes(e.key)){ const i=+e.key-1; if(S.TRACKS[i]){ S.subject=S.TRACKS[i].reg; subjEl.value=S.subject;
