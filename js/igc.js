...
export function parseTz(s){ const m=/([+-]\d{2})(\d{2})/.exec(s||''); return m?(+m[1])+(+m[2])/60:0; }

// Parse an IGC flight log into [lon, lat, gpsAlt, secondsOfDay] points (B-records only).
export function parseIGC(txt){
  const pts=[];
