...
// with UPNG (a pure-JS PNG decoder, also a UMD global) on the main thread.
import { S } from './state.js';
import { TERRAIN, TEXTURE, TERRAIN_N } from './config.js';
import { TileLayer, SimpleMeshLayer, COORDINATE_SYSTEM } from './deck-api.js';

// Geographic bounding box of a web-mercator tile.
export function tileBBox(x,y,z){ const n=2**z;
  const lng=xx=>xx/n*360-180, lat=yy=>{const m=Math.PI*(1-2*yy/n);return 180/Math.PI*Math.atan(Math.sinh(m));};
...
export function buildTerrainMesh(t, west, south, east, north){
  ...
  const elevAt=(px,py)=>{const i=(py*w+px)*4; return rgba[i]*256+rgba[i+1]+rgba[i+2]/256-32768;};
  ...
  return {attributes:{POSITION:{value:positions,size:3},NORMAL:{value:normals,size:3},TEXCOORD_0:{value:texCoords,size:2}},
...
export function makeTerrain(){ return new TileLayer({
  id:'terrain', data:TERRAIN, minZoom:0, maxZoom:13, tileSize:256, maxCacheSize:300,
  getTileData: async (tile)=>{ const {x,y,z}=tile.index||tile;
    const url=TERRAIN.replace('{z}',z).replace('{x}',x).replace('{y}',y);
    try{ const buf=await fetch(url,{signal:tile.signal}).then(r=>r.arrayBuffer());
      const img=window.UPNG.decode(buf); return {rgba:new Uint8Array(window.UPNG.toRGBA8(img)[0]), w:img.width, h:img.height};
    }catch(e){ return null; } },
  renderSubLayers: (props)=>{ const t=props.data; if(!t) return null;
    const {x,y,z}=props.tile.index, bb=tileBBox(x,y,z);
    const turl=TEXTURE.replace('{z}',z).replace('{x}',x).replace('{y}',y);
    return new SimpleMeshLayer({ id:String(props.id)+'-m', data:TERRAIN_ANCHOR, getPosition:()=>[0,0,0],
      _instanced:false, coordinateSystem:COORDINATE_SYSTEM.LNGLAT,
      mesh: buildTerrainMesh(t, bb.west, bb.south, bb.east, bb.north), texture:turl,
