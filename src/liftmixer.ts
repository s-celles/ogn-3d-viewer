...
// Box for n vertices — the SVG viewport the simplex is drawn in.
...
const verts = (n: number, G: { CX: number; CY: number; R: number }): [number, number][] =>
  simplexVerts(n, G.CX, G.CY, G.R);
