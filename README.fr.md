...
- **Chargement** — recherche d'aérodrome (code OACI ou national/FAA), **mode direct** temps réel, import de traces **IGC / GPX / KML**, de waypoints SeeYou **`.cup`** et de polaires **`.plr`** (XCSoar/LK8000), **découverte de spots** mondiaux par continent (records, championnats) — la liste est un [Tabular Data Package](data/spots/spots.csv) maintenu par `just check-spots` — et **points chauds en direct** classant où les planeurs volent en ce moment. L'appli s'ouvre sur cette page Découvrir (l'écran d'accueil) ; cliquer un point chaud charge la journée de l'aérodrome réel le plus proche — même les zones nommées d'après un récepteur OGN sont résolues vers un aérodrome FlightBook.
- **Scène 3D** — relief avec imagerie satellite (résolution réglable) et fond de carte au choix, un détail **IGN RGE ALTI / BD ORTHO** plus fin sur la France (*expérimental*), trois vues (ensemble, cockpit, poursuite), HUD, cône de finesse, ombres au sol, rideau d'altitude, étiquettes par aéronef, des **points d'intérêt** — sommets OSM nommés et waypoints `.cup` importés, avec icônes par type (aérodrome, vachable, sommet, obstacle, repère) — une **mini-carte 2D**, un **HUD en vue d'ensemble** optionnel pour l'aéronef focalisé, et un filtre **aéronefs actifs seulement**.
...
- **Instruments & trafic** — attitude estimée, vario compensé (énergie totale), **netto / super netto** en option (d'après une polaire de planeur importable), son du vario, radar cap-en-haut ou anti-collision directionnel.
...
📖 **Guide détaillé des fonctionnalités** : bouton **📖 Guide** dans le panneau **ⓘ**, ou [`docs/features.fr.md`](docs/features.fr.md).
